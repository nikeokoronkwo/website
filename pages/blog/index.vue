          ...
          <div v-for="m in items" :key="m.title" class="px-5 flex flex-row justify-between py-5 border rounded-lg border-primary-900 overflow-hidden shadow-sm max-h-80">
            <div class="flex flex-col space-y-2 justify-evenly max-w-lg">
              ...
              <div class="justify-end flex flex-col items-end">
                <NuxtLink :to="`/blog${m._path}`" class="transition ease-in-out delay-150 duration-500 border rounded-lg border-transparent hover:border-primary-900 hover:shadow px-5 py-1 text-lg">
                    Read
                </NuxtLink>
              </div>
          </div>
