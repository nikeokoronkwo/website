...
const { data, status, error, refresh, clear } = await useAsyncData("project", () =>
  queryContent(
    `/projects`,
  )
...
<template>
  <div class="min-h-lvh flex flex-col min-w-lvw">
    <div class="flex flex-row justify-between items-center pl-2 bg-primary-950 w-lvw text-primary-50 py-5">
      <div class="flex flex-col items-start text-start space-y-5 px-5 py-6">
        ...
        <div class="text-gray-300">{{ projectInfo?.description ?? "No description available" }}</div>
      </div>
      <div class="flex flex-col items-end text-start space-y-5 px-5">
        <div class="items-center justify-center grid grid-cols-2 text-primary-50">
          <IconList :list="projectInfo?.directs" dark-mode />
        </div>
        <div class=" self-end align-bottom">
          <div class="font-medium text-sm whitespace-nowrap overflow-hidden pb-3">
            Languages Used
          </div>
          <div class="grid grid-cols-2">
            <div v-for="l in projectInfo?.languages ?? []"
              :key="l">
            <img
            ...
            />
            <div ref="tooltip" id="tooltip-default" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
