            ...
            {project.inProgress
              ? (
                <>
                  ...
                  <div className="self-end text-sm text-primary-950 italic mt-auto pb-1">in progress</div>
                </>
              )
              : project.name}
          </div>
          <p className="text-gray-700 text-base py-5">
            {project.description ? (typeof project.description === "string" ? project.description : <project.description />) : (
        ...
        {project.directs ? <div className="self-end px-3 py-3">
          ...
          {!project.route ? <></> : (
            <Route to={`/projects/${project.route}`}>
              <button 
                  className="transition ease-in-out delay-150 duration-500 border rounded-lg border-transparent hover:border-primary-900 hover:shadow px-5 py-1"
