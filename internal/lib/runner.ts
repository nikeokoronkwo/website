...
export class Runner {
    ...
    async run(cmd: string, args: string[]) {
        ...
        if (code != 0) {
            this.logger.error(`Error running command: ${commandStr} exited with exit code ${code}`);
            if (this.logger instanceof VerboseLogger) this.logger.verbose(`Stderr: ${italic(textDecoder.decode(stderr))}`);
